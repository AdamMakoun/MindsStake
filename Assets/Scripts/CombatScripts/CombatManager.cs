...
public class CombatManager : MonoBehaviour
{
    ...
    [SerializeField] Animation dmgAnim;
    ...
    public void startCombat(Player player, GameObject enemyObject, Enemy enemyScript)
    {
        ...
        updateBars();
        AmbienceAudioManager.Instance.PlayStartCombatSound();
    }
    public void Run()
    {
        RunFromCombat();
    ...
    {
        enemy.SelectBodyPartAndDamageIt(player.finalAtkStat * (1 - player.StressLevel / player.MaxStressLevel), "Right Arm");
        if (enemy.CheckForDeath()) StopCombat();
        else BeginAttackSequence();
    }
    public void attackEnemyLeftArm()
    {
        enemy.SelectBodyPartAndDamageIt(player.finalAtkStat * (1 - player.StressLevel / player.MaxStressLevel), "Left Arm");
        if (enemy.CheckForDeath()) StopCombat();
        else BeginAttackSequence();
    }
    public void attackEnemyRightLeg()
    {
        enemy.SelectBodyPartAndDamageIt(player.finalAtkStat * (1 - player.StressLevel / player.MaxStressLevel), "Right Leg");
        if (enemy.CheckForDeath()) StopCombat();
        else BeginAttackSequence();
    }
    public void attackEnemyLeftLeg()
    {
        enemy.SelectBodyPartAndDamageIt(player.finalAtkStat * (1 - player.StressLevel / player.MaxStressLevel), "Left Leg");
        if (enemy.CheckForDeath()) StopCombat();
        else BeginAttackSequence();
    }
    public void attackEnemyBody()
    {
        enemy.SelectBodyPartAndDamageIt(player.finalAtkStat * (1 - player.StressLevel / player.MaxStressLevel), "Body");
        if (enemy.CheckForDeath()) StopCombat();
        else BeginAttackSequence();
    }
    public void attackPlayer()
    {
        ...
        if (!enemy.CheckForDeath()) {
            ...
            dmgAnim.Play();
            AmbienceAudioManager.Instance.PlayHitInCombatSound();
        }
    ...
    {
        GameManager.instance.SetIntensity();
